...
class GeminiPipeline:
    ...
    def infer_with_file(self, file_path):
        ...
        labels = Label if matchs == 1 else conclusion.strip()
        meta_dict["raw_predict"] = result
        meta_dict["think"] = think.strip()
        meta_dict["conclusion"] = conclusion.strip()
        ...
        if result is None:
            ...
            data_dict = {
                "messages": [
                    ...
                    {
                        "content": f"<think>{think.strip()}</think><conclusion>{labels}</conclusion>",
                        "role": "assistant"
                    }
                    ],
                    f"{Type}s": [
    ...
    def pipeline_with_single_line(self, unzip_path, to_path, **kwargs):
        """
        流水线开始
        """
        if os.path.exists(to_path):
            ...
            to_data = Tools.deduplicate_by_id(to_data)
            uid_map = {i["rawInfer"]["id"]: i for i in to_data if i["rawInfer"]["error"] != 1}
            uids = uid_map.keys()
        else:
            uids = []
            to_data = []
        ...
        for file_path in all_files:
            while True:
                try:
                    ...
                    logger.error(f"{file_path} error is {str(SE)}")
                    if "429 RESOURCE_EXHAUSTED" in f"{str(SE)}":
                      sys.exit(1)
            if result is not None:
                to_data.append(result)
                if len(to_data) % 10 == 0:
                    logger.info(f"已经处理了{len(to_data)}.")
                    Tools.to_json(to_data, to_path)
...
if __name__ == "__main__":
    file_root = "interpretability/image/test" ## modelscope的路径
    to_root = "/content/drive/MyDrive/Project/AIGC-Detection" ## 保存路径
    ...
    for file_path in files:
        ...
        geminiPipeline.pipeline_with_single_line(
            unzip_path, # 原始图片路径
